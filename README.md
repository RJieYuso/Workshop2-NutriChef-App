...
|---|---|---|---|
| Food Freshness Detection | Computer vision (TensorFlow / Keras) | Local/backend model | Classifies food from a photo as **fresh / rotten** across 10 food categories |
...
**Model design**
- Transfer learning on **MobileNetV2**, chosen for its high efficiency and low latency on mobile hardware, with a custom classification head (GlobalAveragePooling2D, Dropout(0.2), Dense(256, ReLU), Dropout(0.2), Dense(softmax)).
- 20-class output covering 10 food categories, each split into `fresh` / `rotten` (e.g. `freshapples`, `rottenapples`, `freshbanana`, `rottenbanana`, …).
...
- **Food freshness scanning** — point the camera at food and get a fresh / rotten verdict with a confidence score.
...
| Computer Vision | Python, TensorFlow / Keras, MobileNetV2 transfer learning |
